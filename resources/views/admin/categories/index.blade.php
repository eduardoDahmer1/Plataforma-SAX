...
<div class="container mt-4">
    ...
    <form action="{{ route('admin.categories.index') }}" method="GET" class="mb-4">
        <div class="d-flex flex-column flex-md-row gap-2">
            <input type="text" name="search" class="form-control" placeholder="Buscar por nome ou slug" value="{{ request('search') }}">
                ...
                <div class="card shadow-sm h-100 d-flex flex-column">
                    <div class="card-body d-flex flex-column">
                        ...
                        <div class="d-flex flex-column flex-md-row gap-2 mt-auto">
                            <a href="{{ route('admin.categories.show', $category) }}" class="btn btn-info btn-sm flex-fill">
                                <i class="fa fa-eye me-1"></i> Ver
                            </a>
                            <a href="{{ route('admin.categories.edit', $category) }}" class="btn btn-warning btn-sm flex-fill">
                                <i class="fa fa-edit me-1"></i> Editar
                            </a>
                            <form action="{{ route('admin.categories.destroy', $category) }}" method="POST" onsubmit="return confirm('Tem certeza?')" class="flex-fill m-0">
                                @csrf
                                @method('DELETE')
