@extends('layout.admin')

@section('content')
<x-admin.card>
{{-- Navegação e Título --}}
    <div class="mb-5">
        <a href="{{ route('admin.payments.index') }}" class="text-decoration-none x-small fw-bold text-uppercase text-secondary tracking-wider">
            <i class="fa fa-chevron-left me-1"></i> {{ __('messages.voltar_metodos_btn') }}
        </a>
        <h1 class="h4 fw-light mt-2 mb-0 text-uppercase tracking-wider">
            {{ __('messages.configurar_gateway_titulo') }}
        </h1>
        <div class="sax-divider-dark mt-3"></div>
    </div>

    <div class="row">
        <div class="col-lg-8">
            <form id="paymentMethodForm" action="{{ isset($method) ? route('admin.payments.update', $method->id) : route('admin.payments.store') }}" method="POST">
                @csrf
                @if(isset($method)) @method('PUT') @endif
                @if ($errors->any())
                    <div class="alert alert-danger border-0 rounded-3 mb-4">
                        <strong>Revise a configuração:</strong>
                        <ul class="mb-0 mt-2">
                            @foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach
                        </ul>
                    </div>
                @endif

                {{-- Seção: Definição Básica --}}
                <section class="mb-5">
                    <h6 class="x-small fw-bold text-uppercase text-secondary tracking-tighter mb-4">{{ __('messages.definicao_geral_sec') }}</h6>
                    <div class="row g-4">
                        <div class="col-md-7">
                            @include('admin.payments._gateway_selector')
                        </div>

                        <div class="col-md-5">
                            <label for="type" class="sax-form-label">{{ __('messages.tipo_conexao_label') }}</label>
                            <select name="type" id="type" class="form-select sax-input" required>
                                <option value="bank" {{ old('type', $method->type ?? 'bank') === 'bank' ? 'selected' : '' }}>{{ __('messages.deposito_transferencia_opt') }}</option>
                                <option value="gateway" {{ old('type', $method->type ?? 'bank') === 'gateway' ? 'selected' : '' }}>{{ __('messages.gateway_automatico_opt') }}</option>
                            </select>
                        </div>
                    </div>
                </section>

                {{-- Seção Dinâmica: Detalhes Bancários --}}
                <section class="mb-5 bank-only border-start border-3 ps-4" style="{{ isset($method) && $method->type === 'gateway' ? 'display:none' : '' }}">
                    <h6 class="x-small fw-bold text-uppercase text-secondary tracking-tighter mb-4">{{ __('messages.dados_transferencia_sec') }}</h6>
                    <div class="row">
                        <div class="col-12">
                            <label for="bank_details" class="sax-form-label">{{ __('messages.instrucoes_pagamento_label') }}</label>
                            <textarea name="bank_details" id="bank_details" class="form-control sax-input" rows="5" 
                                     placeholder="{{ __('messages.instrucoes_pagamento_placeholder') }}">{{ $method->bank_details ?? '' }}</textarea>
                            <small class="text-muted x-small mt-2 d-block italic">{{ __('messages.exibicao_cliente_hint') }}</small>
                        </div>
                    </div>
                </section>

                {{-- Seção Dinâmica: Credenciais Gateway --}}
                <section class="mb-5 gateway-only generic-gateway-only border-start border-3 border-dark ps-4" style="{{ isset($method) && $method->type === 'gateway' && !($method->is_rendix_pix ?? false) ? '' : 'display:none' }}">
                    <h6 class="x-small fw-bold text-uppercase text-dark tracking-tighter mb-4">{{ __('messages.credenciais_api_sec') }}</h6>
                    <div class="row g-4">
                        <div class="col-md-6">
                            <label for="public_key" class="sax-form-label">Public Key</label>
                            <input type="text" name="public_key" id="public_key" class="form-control sax-input font-monospace small" 
                                   value="{{ $method->credentials['public_key'] ?? '' }}">
                        </div>
                        <div class="col-md-6">
                            <label for="private_key" class="sax-form-label">Private Key / Token</label>
                            <input type="password" name="private_key" id="private_key" class="form-control sax-input font-monospace small" 
                                   value="{{ $method->credentials['private_key'] ?? '' }}">
                        </div>
                        <div class="col-12" id="sandboxControl">
                            <div class="form-check form-switch mt-2">
                                <input type="checkbox" name="sandbox" value="1" class="form-check-input cursor-pointer" id="sandbox" {{ (isset($method) && ($method->settings['sandbox'] ?? false)) ? 'checked' : '' }}>
                                <label class="form-check-label x-small fw-bold text-uppercase ms-2 cursor-pointer" for="sandbox">
                                    {{ __('messages.usar_sandbox_label') }}
                                </label>
                            </div>
                            <small class="text-muted x-small mt-2 d-block italic">{{ __('messages.sandbox_hint') }}</small>
                        </div>
                    </div>
                </section>

                @include('admin.payments._rendix_fields')

                {{-- Configurações de Ativação --}}
                <div class="border-top pt-4 mt-5 d-flex align-items-center justify-content-between">
                    <div class="form-check form-switch">
                        <input type="checkbox" name="active" value="1" class="form-check-input cursor-pointer" id="active" 
                               {{ old('active', isset($method) && $method->active) ? 'checked' : '' }}>
                        <label class="form-check-label x-small fw-bold text-uppercase ms-2 cursor-pointer" for="active">
                            {{ __('messages.habilitar_metodo_checkout') }}
                        </label>
                    </div>

                    <button type="submit" class="btn btn-dark rounded-0 px-5 py-2 fw-bold text-uppercase tracking-wider small">
                        {{ __('messages.guardar_alteracoes_btn') }}
                    </button>
                </div>
            </form>
        </div>
    </div>

    <div class="modal fade" id="confirmPaymentChangesModal" tabindex="-1" aria-labelledby="confirmPaymentChangesTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header border-0 pb-0">
                    <h2 class="modal-title fs-5 fw-bold" id="confirmPaymentChangesTitle">Você tem certeza?</h2>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                </div>
                <div class="modal-body pt-3">
                    <div id="paymentEnvironmentAlert" class="alert mb-3" role="alert">
                        <strong id="paymentEnvironmentTitle" class="d-block mb-1"></strong>
                        <span id="paymentEnvironmentMessage"></span>
                    </div>
                    <p class="mb-0 text-secondary">
                        Confirme para guardar as alterações deste método de pagamento.
                    </p>
                </div>
                <div class="modal-footer border-0 pt-0">
                    <button type="button" class="btn btn-outline-secondary px-4" data-bs-dismiss="modal">Não</button>
                    <button type="button" class="btn px-4 fw-bold" id="confirmPaymentChangesButton">Sim, guardar</button>
                </div>
            </div>
        </div>
    </div>

</x-admin.card>
@endsection
