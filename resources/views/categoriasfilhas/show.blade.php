...
@section('content')
    <div class="category-detail-wrapper">
        ...
        @php
            $bannerUrl = null;
            // 1. Tenta o banner específico da categoria filha
            if ($categoriasfilhas->banner && Storage::disk('public')->exists($categoriasfilhas->banner)) {
                $bannerUrl = Storage::url($categoriasfilhas->banner);
            } 
            // 2. Fallback para o banner global
...
<style>
    ...
    .category-name-text { font-weight: 300; text-transform: uppercase; letter-spacing: 5px; color: #000; margin: 0; }
    ...
    .child-breadcrumb { font-size: 0.65rem; color: #aaa; text-transform: uppercase; letter-spacing: 1px; }
    ...
    .pagination-sax .page-link { color: #000; border: none; background: transparent; font-size: 0.8rem; }
    .pagination-sax .page-item.active .page-link { background: none; text-decoration: underline; font-weight: bold; }
