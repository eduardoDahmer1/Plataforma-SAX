            ...
            @if ($show && $products->isNotEmpty())
                                    ...
                                    <div class="card h-100 shadow-sm border-0 position-relative">
                                        ...
                                        <div class="card-body p-2 d-flex flex-column">
                                            <h6 class="card-title mb-2">
                                                <a href="{{ route('produto.show', $item->id) }}" class="text-decoration-none">{{ $item->external_name }}</a>
                                            </h6>
                                            ...
                                            @if ($item->stock > 0)
                                                <span class="badge bg-success"><i class="fas fa-box me-1"></i>{{ $item->stock }} em estoque</span>
                                            @else
                                                <span class="badge bg-danger"><i class="fas fa-times-circle me-1"></i>Sem estoque</span>
                                            @endif

                                            <div class="mt-auto d-flex flex-column">
                                                <a href="{{ route('produto.show', $item->id) }}" class="btn btn-sm btn-info mt-2 mb-2">
                    ...
                    <div class="row mb-4">
                        @foreach ($products as $item)
                            <div class="col-6 col-md-3 mb-4">
                                <div class="card h-100 shadow-sm border-0 position-relative">
                                    <img src="{{ $item->photo_url }}" class="card-img-top" alt="{{ $item->external_name }}" style="max-height:150px; object-fit:scale-down;">
                                    @auth
                                        @php $currentQty = $cartItems[$item->id] ?? 0; @endphp
                                        <form action="{{ route('user.preferences.toggle') }}" method="POST" class="card-favorite-form d-none">
                                            @csrf
                ...
                @if ($show && isset($banners[$loop->index]) && $banners[$loop->index])
                    <div class="my-4 text-center">
                        <img src="{{ asset('storage/uploads/' . $banners[$loop->index]) }}" alt="Banner {{ $loop->index + 1 }}" class="img-fluid rounded banner-img">
                    </div>
                @endif
            @endif
        @endforeach
                        ...
                        <div class="swiper-slide">
                            <div class="card h-100 shadow-sm border-0 rounded-3 overflow-hidden">
                                ...
                                <div class="card-body d-flex flex-column">
                                    ...
                                    <p class="card-text text-muted flex-grow-1">{{ Str::limit($blog->subtitle, 80) }}</p>
                                    <div><small class="badge bg-secondary">{{ $blog->category->name ?? 'Sem categoria' }}</small></div>
