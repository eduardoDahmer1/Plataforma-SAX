...
@section('content')
    <div class="container py-4">
        ...
        @foreach ($highlightTitles as $key => $title)
            ...
            @if ($products->isNotEmpty())
                ...
                @if (in_array($key, ['destaque', 'melhores_avaliacoes', 'lancamentos', 'tendencias']))
                    ...
                    <div id="slider-{{ $key }}" class="carousel slide mb-4" data-bs-ride="carousel"
                        data-bs-touch="true">
                        ...
                        {{-- Setas pretas --}}
                        <button class="carousel-control-prev" type="button" data-bs-target="#slider-{{ $key }}"
                            data-bs-slide="prev">
                            <span class="carousel-control-prev-icon custom-arrow"></span>
                        </button>
                        <button class="carousel-control-next" type="button" data-bs-target="#slider-{{ $key }}"
                            data-bs-slide="next">
        ...
        @if (isset($blogs) && $blogs->isNotEmpty())
            ...
            <div id="blogSlider" class="carousel slide mb-4" data-bs-ride="carousel" data-bs-touch="true">
                <div class="carousel-inner">
                    @foreach ($blogs->chunk(3) as $chunkIndex => $chunk)
                        <div class="carousel-item {{ $chunkIndex === 0 ? 'active' : '' }}">
                            <div class="row">
                                            ...
                                            <div class="ratio ratio-16x9 bg-light">
                                                @if ($blog->image && Storage::disk('public')->exists($blog->image))
                                                    <img src="{{ Storage::url($blog->image) }}"
                                                        alt="{{ $blog->title }}" class="img-fluid object-fit-coverr">
                                                @else
                                                    <img src="{{ asset('storage/uploads/noimage.webp') }}"
                                                        alt="Imagem padrão" class="img-fluid object-fit-coverr">
                                                @endif
                                            </div>
