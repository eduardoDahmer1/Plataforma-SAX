                ...
                <form action="{{ route('checkout.deposito.submit', $order->id) }}" method="POST"
                    ...
                    @if ($order->deposit_receipt)
                        <p class="text-success mt-2">Comprovante enviado:
                            <a href="{{ asset('storage/' . $order->deposit_receipt) }}" target="_blank" class="fw-bold">Ver
                                arquivo</a>
                        </p>
                    @endif

                    <button type="submit" class="btn btn-success w-100 mt-3"><i class="fa fa-check-circle"></i> Enviar
                        Comprovante</button>
                </form>
            </div>
        </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-2 p-2 border-bottom">
                        ...
                        <div class="text-end">
                            ...
                            <small class="text-muted">Total:
                                {{ currency_format(($item->product->price ?? 0) * $item->quantity) }}</small>
                        </div>
                    </div>
                    @php $totalPedido += ($item->product->price ?? 0) * $item->quantity; @endphp
                @endforeach
                <hr>
                <h5 class="text-end"><i class="fa fa-money-bill-wave"></i> Total do Pedido:
                    {{ currency_format($totalPedido) }}</h5>
            </div>
        </div>
