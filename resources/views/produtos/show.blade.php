        ...
        <div class="row g-0">
            ...
            <div class="col-lg-6 p-4 bg-light d-flex flex-column justify-content-center">
                ...
                <div class="ratio ratio-1x1 mb-3 rounded-3 shadow-sm overflow-hidden">
                    <img src="{{ $mainImage }}" class="img-fluid w-100 h-100 object-fit-contain" alt="{{ $product->external_name }}">
                </div>
                ...
                @if(count($gallery))
                    <div class="d-flex gap-2 overflow-auto py-2">
                        @foreach($gallery as $img)
                            <img src="{{ Storage::url($img) }}" class="img-thumbnail rounded-3" style="height: 60px; cursor: pointer;" onclick="document.querySelector('.ratio img').src='{{ Storage::url($img) }}'">
                        @endforeach
                    </div>
                @endif
            </div>
            ...
            <div class="col-lg-6 p-4 d-flex flex-column justify-content-between">
                <div>
                    ...
                    <div class="d-flex flex-wrap align-items-center mb-3 gap-1 small">
                        @if($product->category)
                            <a href="{{ route('categories.show', $product->category->slug) }}" class="text-dark fw-semibold text-decoration-none">{{ $product->category->name }}</a>
                        @else
                        ...
                        @if($product->subcategory)
                            ...
                            <a href="{{ route('subcategories.show', $product->subcategory->id) }}" class="text-dark fw-semibold text-decoration-none">{{ $product->subcategory->name }}</a>
                        @endif

                        @if($product->childcategory)
                            <span class="text-dark">›</span>
                            <a href="{{ route('childcategories.show', $product->childcategory->slug) }}" class="text-dark fw-semibold text-decoration-none">{{ $product->childcategory->name }}</a>
                        @endif
                    </div>
                ...
                <div class="mt-4 d-flex flex-wrap gap-2">
                    @auth
                        ...
                        <form action="{{ route('user.preferences.toggle') }}" method="POST" class="d-inline">
                            @csrf
                            <input type="hidden" name="product_id" value="{{ $product->id }}">
                            <button type="submit" class="btn {{ $isFavorite ? 'btn-danger' : 'btn-outline-danger' }} px-4 shadow-sm">
                                <i class="fas {{ $isFavorite ? 'fa-heart-broken' : 'fa-heart' }} me-1"></i>
                                {{ $isFavorite ? 'Remover dos Favoritos' : 'Adicionar aos Favoritos' }}
                            </button>
                        </form>
                    @else
                        <a href="#" class="btn btn-warning px-4 shadow-sm" data-bs-toggle="modal" data-bs-target="#loginModal">
