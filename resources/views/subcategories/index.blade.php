        ...
        <div class="col-md-4 mb-4">
            <div class="card h-100">
                <div class="card-img-top text-center">
                    ...
                    @else
                        <img src="{{ asset('storage/uploads/noimage.webp') }}" alt="Sem imagem" class="img-fluid rounded-3 shadow-sm">
                    @endif
                </div>
                <div class="card-body text-center">
                    <h5>{{ $subcategory->name }}</h5>
                    <small class="text-muted">Categoria Pai: {{ $subcategory->category->name ?? 'N/A' }}</small><br>
                    <a href="{{ route('subcategories.show', $subcategory->id) }}" class="btn btn-primary mt-2">Ver detalhes</a>
                </div>
            </div>
        </div>
        @endforeach
