@extends('layout.layout')

@section('content')
<div class="container">
    <a href="{{ route('pages.contato') }}" class="btn btn-link">Contato</a>
    <a href="{{ route('pages.sobre') }}" class="btn btn-link">Sobre Nós</a>
    <a href="{{ route('pages.home') }}" class="btn btn-link">Home</a>

    <h1>Lista de Arquivos</h1>

    @if(session('success'))
    <div class="alert alert-success">{{ session('success') }}</div>
    @endif

    <!-- Botão para criar novo upload -->
    <a href="{{ route('uploads.create') }}" class="btn btn-success mb-3">Novo Upload</a>

    <table class="table">
        <thead>
            <tr>
                <th>Título</th>
                <th>Descrição</th>
                <th>Arquivo</th>
                <th>Ações</th>
            </tr>
        </thead>
        <tbody>
            @foreach($uploads as $upload)
            <tr>
                <td>{{ $upload->title }}</td>
                <td>{{ $upload->description }}</td>
                <td><a href="{{ asset('storage/' . $upload->file_path) }}" target="_blank">{{ $upload->original_name }}</a></td>
                <td>
                    <a href="{{ route('uploads.edit', $upload->id) }}" class="btn btn-sm btn-warning">Editar</a>
                    <form action="{{ route('uploads.destroy', $upload->id) }}" method="POST" style="display:inline-block">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-sm btn-danger" onclick="return confirm('Tem certeza?')">Excluir</button>
                    </form>
                </td>
            </tr>
            @endforeach
        </tbody>
    </table>
</div>
@endsection
