...
@section('content')
    ...
    <ul class="list-group mb-4">
        ...
        @if (auth()->user()->phone_country || auth()->user()->phone_number)
            <li class="list-group-item"><i class="fas fa-phone me-2"></i>Telefone: {{ auth()->user()->phone_country ?? '' }}
        ...
        @if (auth()->user()->additional_info)
            <li class="list-group-item"><i class="fas fa-id-card me-2"></i>Número do Cadastro:
                {{ auth()->user()->additional_info }}</li>
        @endif

        @if (auth()->user()->document)
            <li class="list-group-item"><i class="fas fa-id-card me-2"></i>Número do Documento:
                {{ auth()->user()->document }}</li>
        @endif
    </ul>
            ...
            @foreach ($orders->take(5) as $order)
                <li
                    ...
                    <div class="mb-2 mb-md-0">
                        <i class="fas fa-box-open me-2"></i>Pedido #{{ $order->id }} -
                    ...
                    <div class="d-flex gap-2 flex-wrap">
                        ...
                        <a href="{{ route('user.orders.show', $order->id) }}" class="btn btn-sm btn-primary"><i
                                class="fas fa-eye me-1"></i>Ver Detalhes</a>
                    </div>
                </li>
            @endforeach
